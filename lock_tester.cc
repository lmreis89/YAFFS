...
void
check_grant(lock_protocol::lockid_t lid, unsigned cli)
{
  pthread_mutex_lock(&count_mutex);
  int x = lid & 0xff;
  if(ct[x] != 0){
  ...
  if(cli==9){
  	    fprintf(stdout, "error: server granted %016llx to client %d\n", lid,cli);
  	    exit(1);
   }
  ct[x] += 1;
  pthread_mutex_unlock(&count_mutex);
...
test5(void *x)
{
	 int i = * (int *) x;
		 ...
		 client=lc[i];
		  ...
		  printf ("test5: client %d acquire a release a concurrent\n", i);
		  for (int j = 0; j < 10; j++) {
			client->acquire(a);
		    check_grant(a,i);
...
main(int argc, char *argv[])
{
	...
	assert (r == 0);
      ...
      for (int i = 0; i < nt; i++) {
    	  if(i!=9) pthread_join(th[i], NULL);
      }
      printf ("%s: threads finished, cancelling client 9\n", argv[0]);
      pthread_cancel(th[9]);
    }
