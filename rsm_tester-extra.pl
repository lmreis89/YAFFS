...
sub check_proposed_n {

  my $log = shift;
  my @nv = `grep "high " $log | cut -f 2 -d " " `;
  foreach my $value (@nv) {
  	  if( ($value % 2) == 0 ) {
        print "Found accepted value for n : $value";
      }
      else{
      	mydie( "Failed: In log $log accepted a value $value not divisible by 2" );
      }
  }
...
sub wait_and_check_expected_view($) {
  ...
  foreach my $port (@$v) {
    wait_for_view_change(paxos_log($port), $in_views{$port}, $port, 120);
  }
  foreach my $port (@$v) {
    my $log = paxos_log($port);
    check_views( $log, \@views );
    check_proposed_n($log)
  }
}
