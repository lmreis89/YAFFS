...
fuseserver_setattr(fuse_req_t req, fuse_ino_t ino, struct stat *attr, int to_set, struct fuse_file_info *fi)
{
  ...
  if (FUSE_SET_ATTR_SIZE & to_set) {
    ...
    int ret = yfs->setattr(ino, attr->st_size);
    if(ret == yfs_client::OK)
        ret = getattr(ino,st);
...
fuseserver_read(fuse_req_t req, fuse_ino_t ino, size_t size,
      off_t off, struct fuse_file_info *fi)  //TODO
{

    std::pair<std::string,bool> bytes_read = yfs->read(ino, size, off);
    const char* buf = bytes_read.first.c_str();

    if(!bytes_read.second)
      fuse_reply_buf(req, buf, bytes_read.first.length());
    else
      fuse_reply_err(req, ENOENT);
}
...
fuseserver_write(fuse_req_t req, fuse_ino_t ino,
  ...
  struct fuse_file_info *fi) //TODO
{

    int bytes_written = yfs->write(ino, buf, size, off);
     ...
     mode_t mode, struct fuse_entry_param *e)
{
  ...
  yfs_client::inum num = yfs->mknod(parent, name);
  (*e).ino = num;
  getattr(num, (*e).attr);
...
fuseserver_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  yfs_client::inum inumb = yfs->ilookup(parent,string_name);

  // If the file was found, initialize e.ino and
  // e.attr appropriately.
  if(inumb != -1)
  {
      found = true;
      e.ino = inumb;
      getattr(inumb, e.attr );
  }
          ...
          off_t off, struct fuse_file_info *fi)
{
    ...
    memset(&b, 0, sizeof(b));
    std::map< std::string , yfs_client::inum> contents;
    std::map< std::string , yfs_client::inum>::iterator it;
    contents = yfs->readDir(ino);

    for(it = contents.begin(); contents.end() != it; it++)
    {
       fuse_ino_t x = (*it).second;
       dirbuf_add(&b, (*it).first.c_str(), x);
    }
...
fuseserver_open(fuse_req_t req, fuse_ino_t ino,
     struct fuse_file_info *fi) //TODO
{
  // é so para verificar q existe?!?
  bool exists = yfs->exists(ino);

  if(exists)
    fuse_reply_open(req, fi);
  else
    fuse_reply_err(req, ENOENT); // fuse_reply_err(req, ENOSYS);
     ...
     mode_t mode)
{
    struct fuse_entry_param e;
    std::string s (name);
    yfs_client::inum inum = yfs->mkDir(parent,s);
    if(inum == -1)
        fuse_reply_err(req, ENOENT);
    e.ino = inum;
...
fuseserver_unlink(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  std::string s (name);
  yfs_client::status status = yfs->unlink(parent,name);
